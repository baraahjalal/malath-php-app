...
<style>
.feed-card { background-color:#ffffff; border-radius:1.5rem; padding:1.5rem; box-shadow:0 4px 20px rgba(0,0,0,0.02); border:1px solid rgba(0,0,0,0.04); margin-bottom:1.5rem; transition:all 0.3s ease; text-align:right; }
...
.post-actions-crud { display:flex; gap:0.8rem; margin-top:1rem; padding-top:1rem; border-top:1px solid rgba(0,0,0,0.04); }
.btn-crud-edit, .btn-crud-delete { background:none; border:none; padding:0.4rem 1rem; border-radius:1.5rem; font-size:0.85rem; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:0.4rem; transition:all 0.2s ease; font-family:inherit; }
.btn-crud-edit { color:#0284c7; background-color:#e0f2fe; }
.btn-crud-edit:hover { background-color:#bae6fd; color:#0369a1; }
.btn-crud-delete { color:#be185d; background-color:#fce7f3; }
.btn-crud-delete:hover { background-color:#fbcfe8; color:#9d174d; }
</style>

<?php if ($status === 'updated'): ?>
<div style="background:#dcfce7;color:#166534;padding:1rem 2rem;text-align:center;font-weight:600;">✅ تم تحديث بياناتك بنجاح.</div>
<?php elseif ($status === 'error'): ?>
<div style="background:#fee2e2;color:#991b1b;padding:1rem 2rem;text-align:center;font-weight:600;">حدث خطأ أثناء التحديث. حاولي مجدداً.</div>
<?php endif; ?>
    ...
    <div class="container relative">
        <div class="profile-info-card animate-fade-in-up">
            <div class="profile-avatar-large">
                <img src="<?= !empty($user['avatar']) ? htmlspecialchars($user['avatar']) : 'assets/images/default-avatar.png' ?>" alt="Profile Picture">
            </div>
            <div class="profile-details flex justify-between items-center w-full">
                <div>
                    <h1 class="font-headline font-black text-primary-dark" style="font-size:2.2rem; margin-bottom:0.25rem;"><?= htmlspecialchars($user['name']) ?></h1>
...
<section style="background-color:#FFFFFF; border-bottom:1px solid var(--outline-variant); position:sticky; top:4.5rem; z-index:30; box-shadow:0 4px 10px rgba(0,0,0,0.02);">
    <div class="container">
        <div class="profile-tabs flex gap-8">
            <button class="profile-tab active" onclick="switchTab(this, 'personal-info')">إعدادات الحساب</button>
            <button class="profile-tab" onclick="switchTab(this, 'contributions')">نشاطاتي في المجتمع</button>
            <button class="profile-tab" onclick="switchTab(this, 'saved-items')">المكتبة الخاصة</button>
        </div>
    </div>
</section>
                            ...
                            <div style="opacity:<?= ($act['activity_type']==='post') ? '1' : '0.85' ?>; padding-right:<?= ($act['activity_type']==='post') ? '0' : '1rem' ?>; border-right:<?= ($act['activity_type']==='post') ? 'none' : '3px solid var(--outline-variant)' ?>;">
                                <div class="post-header" style="margin-top:1rem;">
                                    <div class="user-info">
                                        ...
                                        <div><b class="user-name" style="font-size:0.9rem;"><?=htmlspecialchars($act['author_name'])?></b></div>
                                    </div>
